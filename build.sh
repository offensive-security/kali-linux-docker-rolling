...
sudo tar -C kali-root -c . | sudo docker import - officialkali/kali-rolling &&\
sudo rm -rf ./kali-root &&\
TAG=$(sudo docker run -t -i officialkali/kali-rolling awk '{print $NF}' /etc/debian_version | sed 's/\r$//' ) &&\
echo "Tagging kali with $TAG" &&\
sudo docker tag officialkali/kali-rolling:latest officialkali/kali-rolling:$TAG &&\
